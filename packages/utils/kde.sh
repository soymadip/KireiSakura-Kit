# Utilities to fine-tune kde-plasma DE.


#
#
#==---------------------------------------------------------------------------------
# NAME:   __change_meta_to_krunner
# ALIAS:  plasma.change.meta.to.krunner
# DESC:   Change meta key to open krunner in KDE Plasma.
# USAGE:  plasma.change.meta.to.krunner
#==---------------------------------------------------------------------------------
__change_meta_to_krunner() {
    ui.prompt "Do you want to change meta key to open krunner?" confirm_meta
    ...
    if [ "$confirm_meta" == "y" ] || [ "$confirm_meta" == "Y" ] || [ -z "$confirm_meta" ]; then
        ...
        log.warn "TIP: To get all possible commands using kwriteconfig5"
        log.warn "run 'qdbus org.kde.kglobalaccel /component/kwin org.kde.kglobalaccel.Component.shortcutNames'."
        # kwriteconfig6 --file ~/.config/kwinrc --group ModifierOnlyShortcuts --key Meta "org.kde.kglobalaccel,/component/kwin,org.kde.kglobalaccel.Component,invokeShortcut,NAME_OF_THE_THING_YOU_WANT_TO_OPEN" for custom shortcut..
...
#
#==---------------------------------------------------------------------------------
# NAME:   __change_wallpaper
# ALIAS:  plasma.change.wallpaper
# DESC:   Change desktop wallpaper in KDE Plasma.
# USAGE:  plasma.change.wallpaper <path_to_wallpaper>
#==---------------------------------------------------------------------------------
__change_wallpaper() {

    local wallpaper_path="$1"
    ui.prompt "Do you want to change wallpaper?" confirm_wallpaper
...
#
#==---------------------------------------------------------------------------------
# NAME:   __change_icon_theme
# ALIAS:  plasma.change.icon.theme
# DESC:   Change icon theme in KDE Plasma.
# USAGE:  plasma.change.icon.theme <icon_theme_name>
#==---------------------------------------------------------------------------------
__change_icon_theme() {
    local icon_theme="$1"
    ui.prompt "Do you want to change icon theme?" confirm_icon_theme
...
#
#==---------------------------------------------------------------------------------
# NAME:   __change_cursor_theme
# ALIAS:  plasma.change.cursor.theme
# DESC:   Change cursor theme and size in KDE Plasma.
# USAGE:  plasma.change.cursor.theme <cursor_theme_name> <cursor_size>
#==---------------------------------------------------------------------------------
__change_cursor_theme() {
    local cursor_theme="$1"
    local cursor_size="$2"

    ui.prompt "Do you want to change cursor theme?" confirm_cursor_theme
...
#
#==---------------------------------------------------------------------------------
# NAME:   __change_sddm_theme
# ALIAS:  plasma.change.sddm.theme
# DESC:   Change SDDM login manager theme.
# USAGE:  plasma.change.sddm.theme <theme_name> [config_file]
...
__change_sddm_theme() {
    ...
    if sudo sed -i "s/^Current=.*/Current=$theme_name/" "$config_file" ; then
        log.success "SDDM theme changed to $theme_name."
    else
        log.error "Failed to change SDDM theme."
    fi
}
#==---------------------------------------------------------------------------------
...
alias plasma.change.meta.to.krunner='__change_meta_to_krunner'
alias plasma.change.wallpaper='__change_wallpaper'
alias plasma.change.icon.theme='__change_icon_theme'
alias plasma.change.cursor.theme='__change_cursor_theme'
alias plasma.change.sddm.theme='__change_sddm_theme'
