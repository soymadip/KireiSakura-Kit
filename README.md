...
> "KireiSakuraKit" is combination of two Japanese words, "Kirei" and "Sakura," along with the suffix "Kit". `Kirei(綺麗)` means "simple" or "clean" & `Sakura(桜)` refers to cherry blossoms. Putting them together, "Kirei-SakuraKit" could be interpreted as a UI framework that emphasizes a beautiful and clean design inspired by the elegance of cherry blossoms. 
...
- As for now this project is very Arch-linux centic as I use EndevourOS, but can be made to use with any distro.
- As I am just learning bash and nowhere near to be an expert, this framework/library is very simple. But I believe this can be a very powerful & useful concept.
